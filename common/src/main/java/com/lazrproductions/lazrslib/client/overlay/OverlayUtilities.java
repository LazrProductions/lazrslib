...
public class OverlayUtilities {
     ...
     */
    public static void drawLabel(@NotNull OverlayProperties p, Vec3 pos, Component text, float textScale,
                                 boolean drawShadow) {
        Font font = Minecraft.getInstance().font;
        PoseStack poseStack = p.getPoseStack();
        BufferSource bufferSource = p.getBufferSource();

        Vector3f cameraPos = p.getCameraPos().toVector3f();
        Camera camera = p.getCamera();

        float labelScale = textScale * 0.025f;
        TextColor col = text.getStyle().getColor();
        int labelColor = col != null ? col.getValue() : 0xFFFFFFFF;
        ...
        font.drawInBatch(text, -font.width(text) / 2f, font.lineHeight / 2f, labelColor, drawShadow, pose, bufferSource, Font.DisplayMode.NORMAL, 0, 15728880);
     ...
     */
    public static void drawGrid(@NotNull OverlayProperties p, Vec3 pos, int width, int length, float cellSize,
                                int color) {
     ...
     */
    public static void drawGridCentered(@NotNull OverlayProperties p, Vec3 center, int width, int length,
     ...
     */
    public static void drawGrid(@NotNull OverlayProperties p, Vec3 pos, Vec3i dimensions, Vec3 cellDimensions,
                                int color) {
        double pX = pos.x();
        double pY = pos.y();
        double pZ = pos.z();
     ...
     */
    public static void drawWireCube(@NotNull OverlayProperties p, Vec3 pos, float size, int color) {
        // bottom
        drawWireSquare(p,
                pos,
                size,
     ...
     */
    public static void drawWireBox(@NotNull OverlayProperties p, Vec3 pos, Vec3 dimensions, int color) {
     ...
     */
    public static void drawWireSquare(@NotNull OverlayProperties p, Vec3 pos, float size, int color) {
        drawLine(p,
                pos.add(0, 0, 0),
                pos.add(size, 0, 0),
                color);
     ...
     */
    public static void drawWireSquareCentered(@NotNull OverlayProperties p, Vec3 pos, float size, int color) {
     ...
     */
    public static void drawLine(@NotNull OverlayProperties p, Vec3 from, Vec3 to, int color) {
        drawLine(p,
     ...
     * Draw a line from one point to another.
     */
    public static void drawLine(@NotNull OverlayProperties p, float fromX, float fromY, float fromZ, float toX,
                                float toY,
                                float toZ, int color) {
    ...
    public static class OverlayProperties {
        ...
        public OverlayProperties(@NotNull Camera camera,
                                 @NotNull Matrix4f viewMatrix,
                                 @NotNull RenderBuffers buffers) {
            this.camera = camera;
            this.buffers = buffers;
            this.stack = new PoseStack();
            this.stack.setIdentity();
            this.stack.mulPose(viewMatrix);
        }
        public OverlayProperties(@NotNull Camera camera,
        ...
        public VertexConsumer getConsumerForLines() {
            final RenderType r = RenderType.debugLineStrip(2);
