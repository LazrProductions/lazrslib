...
public class ComponentUtilities {
     ...
     */
    public static int getTotalHeight(@NotNull Minecraft instance, List<Component> list, int width) {
     ...
     * @return The total height in pixels.
     */
    public static int getTotalHeight(@NotNull Font font, List<Component> list, int width) {
        int totalHeight = 0;
     ...
     */
    @NotNull
    public static Component substringJson(String json, @NotNull HolderLookup.Provider provider, int endIndex) {
        try {
            Component comp = Component.Serializer.fromJson(json, provider);
            if (comp != null) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    for (int letter = 0; letter < list.get(i).getString().length(); letter++) {
                        if (currentIndex < endIndex)
                            result.append(Component.literal(list.get(i).getString().substring(letter, letter + 1))
                                    .withStyle(list.get(i).getStyle()));
                        currentIndex++;
                    }
                }
                return result;
     ...
     */
    @NotNull
    public static Component parseFromJson(String json, @NotNull HolderLookup.Provider provider) {
        try {
            Component comp = Component.Serializer.fromJson(json, provider);
            if (comp != null)
                return comp;
