...
public abstract class LazrConfig implements ILazrConfig {
    ...
    public <T> IConfigProperty<T> createGenericProperty(@Nonnull IConfigProperty<T> property) {
        genericProperties.add(property);
        return property;
    }
    public IConfigCategory createCategory(@Nonnull IConfigCategory category, ICreatePropertiesFunction createProperties) {
